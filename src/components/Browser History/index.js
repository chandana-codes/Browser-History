...
class BrowserHistory extends Component {
  ...
  deleteHistoryItem = id => {
    ...
    const deleteItem = latestHistoryList.filter(
      eachHistory => eachHistory.id !== id,
    )

    this.setState({latestHistoryList: deleteItem})
  }

  render() {
    const {searchInput, latestHistoryList} = this.state
    const newHistoryList = latestHistoryList.filter(eachList =>
    ...
    return (
      <div className="container">
        <Navbar
          ...
          onSearchHistory={this.onSearchHistory}
        />
        <History
